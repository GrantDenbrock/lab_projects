...
def parse_args():
    ...
    parser.add_argument(
        '--input_file', required = True,  help = 'A text file from chromstats.')

    parser.add_argument(
        '--male_list', nargs = '+', help='Enter each male sample seperated by a space. For example: "sample1_dna.ref1.mkdup.sorted.bam sample2_dna.ref1.mkdup.sorted.bam". These will be fed into a list used to calculate the stats. Use ""--female_list" to enter the female samples.')
    ...
    return args
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#FIXME need to put all in main now
...
def main():
    ...
    df_ratio_less_than = df_no_nan[df_no_nan.f_m_ratio_cov < float(args.coverage_cutoff)]
